...
import { Check } from 'lucide-react'; // Import icon centang

const Dropdown = ({ icon, label, options = [], onSelect }) => {
  const [open, setOpen] = useState(false);
  const [selectedOption, setSelectedOption] = useState(null); // State untuk menyimpan pilihan
  const ref = useRef();
  ...
  return (
    <div className="relative inline-block" ref={ref}>
      ...
      {open && <DropdownList options={options} onSelect={handleSelect} selectedOption={selectedOption} />}
    </div>
  );
};
...
/** Dropdown List **/
const DropdownList = ({ options, onSelect, selectedOption }) => (
  <ul className="absolute z-10 mt-2 min-w-max bg-white dark:bg-black border-2 border-black dark:border-white rounded-lg shadow-[4px_4px_0_0_#000] dark:shadow-[4px_4px_0_0_#fff] py-1 max-h-60 overflow-auto">
    {options.length === 0 ? (
      <li className="text-gray-500 cursor-default select-none relative py-2 px-4">Tidak ada opsi</li>
    ) : (
      options.map((option) => <DropdownOption key={option.value} option={option} onSelect={onSelect} isSelected={selectedOption && selectedOption.value === option.value} />)
    )}
  </ul>
);

/** Dropdown Option **/
const DropdownOption = ({ option, onSelect, isSelected }) => (
  <li onClick={() => onSelect(option)} className="cursor-pointer select-none relative py-2 pl-3 pr-4 flex items-center gap-2 hover:bg-gray-200 dark:hover:bg-gray-800">
    ...
    {isSelected && <Check className="w-4 h-4 text-green-500" />} {/* Centang jika terpilih */}
  </li>
);
